...
public class UndefinedSKOSResourcesTest {
	...
	@Autowired
	private UndefinedSKOSResources undefinedSKOSResources;

	@Test
	public void testSkosWithRelationClashes() {
		undefinedSKOSResources.reset();
		...
		executionParameters.put(TestParameters.Values.VOCABULARY, "q23_invalid");
		undefinedSKOSResources.setExecutionParameters(executionParameters);
		undefinedSKOSResources.run();
		Result result = undefinedSKOSResources.getResult();

		Assert.assertEquals("There should be exactly 12 errors.", 12, result.getErrorCount());
		String uri = "http://www.w3.org/2004/02/skos/core#subject";
