...
public class MappingClashesTest {
	...
	public void testSkosWithRelationClashes() {
		...
		executionParameters.put(TestParameters.Values.VOCABULARY, "q26_invalid");
		mappingClashes.setExecutionParameters(executionParameters);
		mappingClashes.run();
		Result result = mappingClashes.getResult();

		Assert.assertEquals("There should be exactly 5 errors.", 5, result.getErrorCount());
		String uri1 = "file:/tmp/21";
		String uri2 = "file:/tmp/22";
		assertContainsMessage(result, Message.Type.ERROR, new String[] {}, new String[] { uri1, uri2 });
