...
public class InconsistentPrefLabels extends AbstractQskosTestCase<Collection<LabelConflict>> {
	...
	protected void populateResult(Collection<LabelConflict> data) {
		...
		while (it.hasNext()) {
			...
			for (LabeledConcept concept : concepts) {
				...
				prefLabelsFound++;
				MessageFormat format = new MessageFormat(i18n.tr("tests.impl.qskos.ipl.result.content"));
			...
			addMessage(new Message(ignored ? Message.Type.IGNORED : Message.Type.ERROR, errorCode,
					i18n.tr("tests.impl.qskos.ipl.result.title", new Object[] { prefLabelsFound }),
